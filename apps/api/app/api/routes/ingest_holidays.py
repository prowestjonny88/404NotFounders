...
from app.services.holiday_service import ensure_holiday_snapshot_fresh, refresh_holiday_snapshot
...
@router.post("/holidays/ensure-fresh")
async def ensure_fresh_holiday_snapshot() -> dict[str, str | int]:
    try:
        envelope = ensure_holiday_snapshot_fresh(max_age_hours=24)
    except ProviderError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    except SnapshotWriteFailed as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
